...
# $ SIGN PREVENTS FLOAT FROM WORKING AS FLOAT IS A NUMERIC DATA TYPE
...
#   print(annual_salary)
# NEEDED TO REMOVE EXTRA PARENTHESES AND FLOAT SOME_DECIMAL_NUMBER IN THE ANNUAL_SUMMARY SINCE IT WAS A STRING
some_decimal_number = "1234.56"
annual_salary = float(some_decimal_number) * 52
print(annual_salary)
...
# Let's use an f-string to help with all these decimal numbers - F STRINGS LET YOU CONCATENATE WITHOUT USING ++++
...
# user inputs name, hours worked each week, and hourly pay
# code produces monthly & annual income

# function to validate if the user input is numeric
def check_for_valid_input(user_answer):
    try: 
        float(user_answer)
        return print()
    except ValueError:
...
hours_per_week = input("How many hours do you work each week?:")
check_for_valid_input(hours_per_week)
hourly_pay = input("What is your hourly pay rate?: $")
check_for_valid_input(hourly_pay)
expected_raise = input("What percentage increase do you expect to see this year?: ")
check_for_valid_input(expected_raise)

# function to calculate an increase in pay
def calc_increased_income (current_earnings, expected_raise):
    return ((float(expected_raise)/100)+1)*current_earnings

# calculation results
weekly_pay = float(hours_per_week) * float(hourly_pay)
annual_pay = weekly_pay * 52
monthly_pay = annual_pay/12

new_weekly_pay = calc_increased_income(weekly_pay, expected_raise)
new_annual_pay = calc_increased_income(annual_pay, expected_raise)
new_monthly_pay = calc_increased_income(monthly_pay, expected_raise)


# Telling the user how much they currently earn
print(f"{user_name}, you currently earn ${weekly_pay:,.2f} per week and ${annual_pay:,.2f} per year. If your pay is increased by {expected_raise}%, your new weekly earnings will be ${new_weekly_pay:,.2f} and your new annual pay will be ${new_annual_pay:,.2f}")

# formatted monthly pay info specifically to show a function that searches the string below
pretty_monthly_pay = f'${monthly_pay:,.2f}'
pretty_new_monthly_pay=f'${new_monthly_pay:,.2f}'
my_string = "Your monthly pay is currently about "+str(pretty_monthly_pay)+ " your new monthly pay would be " + str(pretty_new_monthly_pay)
print(my_string)

# grabbing the first $ amount from the string above using the function provided in the body of the code challenge

def find_index(find, string):
    find = string.index(" ")
    return find

print (my_string[find_index("$",my_string):(my_string.find(" ",find_index("$",my_string)))])